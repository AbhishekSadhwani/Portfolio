...
export const Experience = () => {
  ...
  return (
    <section className='border-b border-gray-200 dark:border-neutral-900 pb-24'>
        <motion.h2 initial={{y:-100,opacity:0}} whileInView={{y:0,opacity:1}} viewport={{ once: true }} transition={{duration:0.5}} className='text-4xl text-center text-neutral-900 my-20 dark:text-neutral-300'>Experience</motion.h2>
        <div>
            {EXPERIENCES.map((experience, index) => (
                <div key={index} className='flex flex-wrap lg:justify-center mb-8'>
                    <motion.div initial={{x:-100,opacity:0}} whileInView={{x:0,opacity:1}} viewport={{ once: true }} transition={{duration:1.5}} className='w-full lg:w-1/4'>
                            <p className="mb-2 text-sm text-neutral-800 dark:text-neutral-400">{experience.year}</p>
                    </motion.div>
                    <div className="w-full max-w-xl lg:w-3/4">
                        <motion.div initial={{x:100,opacity:0}} whileInView={{x:0,opacity:1}} viewport={{ once: true }} transition={{duration:1.5}} className="flex flex-col">
                            <p className="text-black font-semibold mb-2 dark:text-white">{experience.role} - <span className="text-sm text-neutral-700 dark:text-purple-100">{experience.company}</span></p>
                            <p className="mb-2 text-neutral-900 dark:text-neutral-400">{experience.description}</p>
